...
function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative flex h-screen overflow-hidden bg-transparent" style={themeVars}>
      ...
      {/* Logo top-left (mobile, when sidebar closed) */}
      {showButton && (
        <button
          type="button"
          onClick={() => window.location.reload()}
          className="fixed left-4 top-4 z-50 flex h-11 w-11 items-center justify-center rounded-xl bg-card/90 shadow-[0_8px_40px_rgba(0,0,0,0.1)] md:hidden"
      ...
      {/* Floating menu button — opens the sidebar drawer (top-right) */}
      {showButton && (
        <button
